...
def getDefaultDependencyResolver(String file) {
    def properties = BuildProperties.getInstance()
	def path = new DependencyPath().sourceDir(properties.sourceDir).directory("HospitalApplication/copybook")
...
def getDefaultImpactResolver(String file) {
	...
	// Add rule to assocuate changes to copybooks with the need to recompile programs that reference them.
   	def cpypath = new DependencyPath().collection(properties.collection).sourceDir(properties.sourceDir).directory("HospitalApplication/copybook")
	...
	// Add rule to associate changes to BMS maps with need to recompile programs that use the generated copybook.
   	def bmspath = new DependencyPath().collection(properties.collection).sourceDir(properties.sourceDir).directory("HospitalApplication/bms")
	...
	// Add rule to associate changes to COBOL source with the need to linkedit .lnk files that reference them.
   	def cobpath = new DependencyPath().collection(properties.collection).sourceDir(properties.sourceDir).directory("HospitalApplication/cobol")
