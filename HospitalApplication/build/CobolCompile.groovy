...
linkedit.dd(new DDStatement().dsn(properties.SDSNLOAD).options("shr"))
if (logicalFile.isCICS()) {
...
// run DB2 Bind PACKAGE if bind is turned on (see HospitalApplication/build/bind.properties)
if (logicalFile.isSQL() && properties.RUN_DB2_BIND.toBoolean()) {
    def scriptName = "$properties.sourceDir/HospitalApplication/build/BindPackage.groovy"
