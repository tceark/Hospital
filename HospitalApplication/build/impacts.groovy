 ...
 * successful HospitalApplication build. The result of the script is the generation of a build list text 
 ...
 * HospitalApplication build.groovy script.
 *
 * If the --buildHash option is omitted or if no previous successful HospitalApplication build exists with a 
 * buildHash property, the contents of HospitalApplication/build/files.txt will be copied to ${workDir}/buildList.txt 
 * resulting in a full build.
 *
 * usage:  impacts.groovy [options]
 *
 *  **NOTE - HospitalApplication/build/build.properties will be loaded (if exists) at script startup.
...
// if buildHash argument omitted, then just copy HospitalApplication/build/files.txt to buildlist and exit
...
// if no lastBuildHash, then just copy HospitalApplication/build/files.txt to buildlist and exit
